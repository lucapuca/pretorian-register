public class Deliberation implements Entity {
	...
	public Deliberation(String deliberation) {
		...
		String id = "N" + extractBetween(deliberation, "N. ", "2013</td>") + "2013";
		this.id = id.replace("<br> ", "");
	}

	private String extractBetween(String deliberation, String start, String end) {
		return deliberation.substring(deliberation.indexOf(start) + 1, deliberation.indexOf(end)).trim();
	}
	...
	public String toString() {
		...
		return buffer.append(id).append(" ").append(title).append(" ").append(link).toString();
	}
}
